...
class OpdPenilaianController extends Controller
{
    public function opdPenilaian(Request $request)
    {
        ...
        $opdPenilaianSebelum = OpdPenilaian::where('year', $data['year'] - 1);
        $opdPenilaian = OpdPenilaian::where('year', $data['year']);

        if ($request->opd_id) {
            $opdPenilaianSebelum->where('opd_id', $data['opd_id']);
            $opdPenilaian->where('opd_id', $data['opd_id']);
        } elseif ($request->data_unit_id) {
            $opdPenilaianSebelum->whereHas('opd', function ($q) use ($data) {
        ...
        if ($opdPenilaian->get()->count() == 0 && $opdPenilaianSebelum->get()->count() == 0) {
            return $this->failedResponse([], 'Data Penilaian Kinerja Organisasi Belum Ada');
        }
        $data = [
            'tahun' => $data['year'],
            'opd' => $opdPenilaian->first()->opd->nama_opd ?? $opdPenilaianSebelum->first()->opd->nama_opd,
        ];
